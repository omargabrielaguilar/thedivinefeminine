...
<x-panel {{ $attributes }}>
    ...
    <div class="font-sans">
        ...
        <a href="{{ $conference->url }}" rel="nofollow" class="hover:text-indigo-500">
            {{ $conference->url }}
        </a>

        @if ($conference->has_cfp && $conference->cfp_url)
            <div class="mt-4 text-gray-500">URL for CFP page:</div>
            <a href="{{ $conference->cfp_url }}" rel="nofollow" class="hover:text-indigo-500">
                {{ $conference->cfp_url }}
            </a>
        @endif
            ...
            <table class="table-auto">
                <tbody>
                    @foreach ($conference->speaker_package->toDisplay() as $category => $amount)
                        @if ($amount != 0)
                            <tr>
                                <td class="pr-4 capitalize">{{ $category }}</td>
                                <td>{{ $amount }}</td>
                            </tr>
                        @endif
                    @endforeach
    ...
    <x-slot name="footer">
        <div>
            ...
            <div>
                {{ $conference->startsAtDisplay() }}
                <span class="text-gray-500">to</span>
                {{ $conference->endsAtDisplay() }}
            </div>
        </div>
        @if ($conference->has_cfp && $conference->cfp_starts_at && $conference->cfp_ends_at)
            <div>
                ...
                <div>
                    {{ $conference->cfpStartsAtDisplay() }}
                    <span class="text-gray-500">to</span>
                    {{ $conference->cfpEndsAtDisplay() }}
                </div>
            </div>
        @endif
