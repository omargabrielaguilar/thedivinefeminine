...
<html class="h-full">
    <head>
        ...
        <meta
            ...
            content="A web app for conference speakers to track talks, bios, and conferences."
        />
    ...
    <body class="h-full">
        ...
        <div id="app" class="{{ $app_bg_color }} flex min-h-full flex-col justify-between">
            <div class="flex-1">
                @include('partials.header', ['title' => $title ?? null])
                @if ($is_home)
                    @yield('content')
                @else
                    <div class="mx-auto bg-indigo-100 p-4 sm:max-w-7xl">
                        @yield('content')
                    </div>
                @endif
            </div>
