...
@section('content')
    <x-panel>
        <div class="flex flex-col items-center sm:flex-none sm:items-start">
            <a href="{{ $user->profile_picture_hires }}">
                <img src="{{ $user->profile_picture_hires }}" class="h-56 w-56 rounded-full" />
            </a>
            ...
            @if ($user->allow_profile_contact)
                <a
                    ...
                    class="text-indigo mt-8 block no-underline"
                >
                    Contact {{ $user->name }}
                </a>
            @endif
    ...
    <x-panel title="Talks" class="mt-6">
        <div class="space-y-4">
            @forelse ($talks as $talk)
                <h4 class="text-indigo text-2xl">
                    <a
                        href="{{ route('speakers-public.talks.show', ['profileSlug' => $user->profile_slug, 'talkId' => $talk->id]) }}"
                    >
                        {{ $talk->currentRevision->title }}
                    </a>
                </h4>
                <span class="text-sm text-gray-500">
                    {{ $talk->currentRevision->length }}-minute {{ $talk->currentRevision->type }} talk at
                    {{ $talk->currentRevision->level }} level
                </span>
            @empty
    ...
    <x-panel title="Bios" class="mt-6">
        <div class="space-y-4">
            ...
            @elseif ($bios->count() == 1)
                <h3 class="mt-8 text-3xl">Bio ({{ $bios->first()->nickname }})</h3>
                <p>{!! str_replace("\n", '<br>', $bios->first()->body) !!}</p>
            @else
                @foreach ($bios as $bio)
                    <h4 class="text-indigo text-2xl">
                        <a
                            href="{{ route('speakers-public.bios.show', ['profileSlug' => $user->profile_slug, 'bioId' => $bio->id]) }}"
                        >
                            {{ $bio->nickname }}
                        </a>
                    </h4>
