...
@section('content')
    ...
    <x-form :action="route('account.edit', $user)" method="PUT" :upload="true">
        <x-panel size="xl" title="User">
            <x-input.text name="email" label="Email Address" type="email" :value="$user->email"></x-input.text>
            ...
            <x-input.text name="name" label="Name" class="mt-8" :value="$user->name"></x-input.text>

            <div class="mt-8">
                <label for="profile_picture" class="mb-2 block font-extrabold text-indigo-900">Profile Picture</label>
                <div class="rounded-full text-center">
                    <img
                        id="profile_picture"
                        src="{{ auth()->user()->profile_picture_hires }}"
                        class="mb-1 h-48 w-48 rounded-full"
                        alt="profile picture"
                    />
                </div>
                @if ($user->profile_picture == null)
                    <x-alert.warning>
                        <strong>Your current public profile picture is sourced from Gravatar.</strong>
                        <br />
        ...
        <x-button.primary type="submit" size="md" class="mt-6">Save</x-button.primary>
    </x-form>
@endsection
