...
@section('list')
    <x-panel size="md" title="User">
        ...
        @if ($user->profile_slug)
            <div class="mt-4">
                ...
                <a href="{{ route('speakers-public.show', $user->profile_slug) }}">
                    {{ route('speakers-public.show', [$user->profile_slug]) }}
                </a>
            </div>
        @endif
    ...
    <x-panel size="md" class="mt-4" title="Dismissed Conferences">
        @forelse ($user->dismissedConferences as $conference)
            <div class="mt-4">
                <a href="{{ route('conferences.show', ['id' => $conference->id]) }}" class="hover:text-indigo-500">
                    {{ $conference->title }}
                </a>
            </div>
        @empty
