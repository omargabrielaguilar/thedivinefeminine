...
@php
    $baseLinkClasses = 'px-5 py-1 hover:bg-indigo-100';
...
@section('list')
    <x-panel size="md" :title="$current->title">
        <x-slot name="subtitle">
            <span class="italic">{{ $current->length }} minute {{ $current->level }} {{ $current->type }}</span>
        </x-slot>
        <x-slot name="actions">
        ...
        <x-panel size="md" title="Submissions">
            @foreach ($submissions as $submission)
                <div class="pr-3 lg:pr-0">
                    <div class="mt-4 flex content-center justify-between">
                        <h3 class="text-lg font-normal text-gray-500">
                            <a
                                class="hover:text-indigo-500"
                                href="{{ route('conferences.show', $submission->conference->id) }}"
                            >
                        ...
                        <a
                            href="{{ route('submission.edit', $submission) }}"
                    ...
                    <div>
                        {{ $submission->conference->startsAtDisplay() }}
                        <span class="text-gray-500">to</span>
                        {{ $submission->conference->endsAtDisplay() }}
                    </div>
                </div>
            @endforeach
        </x-panel>
