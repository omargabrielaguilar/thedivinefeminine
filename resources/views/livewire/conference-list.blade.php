<div>
    <div class="flex space-x-10">
        <x-input.text name="search" placeholder="Search conferences" wire:model="search" class="flex-1" />
        <x-input.select
            :inline="true"
            name="filter"
            label="Filter by"
    ...
    @forelse ($conferences as $month => $conferences)
        <div class="mt-8 flex justify-between">
            <h2 class="text-2xl font-semibold leading-8 text-indigo-600">
    ...
    @if (auth()->user())
        <div class="mt-4 text-right">
            <x-button.primary :href="route('conferences.create')" icon="plus">
